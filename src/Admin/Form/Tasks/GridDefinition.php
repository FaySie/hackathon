	...
	protected function doDefine(Form $form)
	{
		...
		$this->group('search', function (Form $form)
		{
			...
			$this->list('field')
				...
				->option(Translator::translate('admin.task.field.title'), 'task.title')
				->option(Translator::translate('admin.task.field.alias'), 'task.alias');
		 ...
		 *
		 * You can override filter actions in TasksModel::configureFilters()
		 */
		$this->group('filter', function(Form $form)
		{
			...
			$this->list('task.state')
				...
				->option('', '')
				->option(Translator::translate('admin.task.filter.state.select'), '')
